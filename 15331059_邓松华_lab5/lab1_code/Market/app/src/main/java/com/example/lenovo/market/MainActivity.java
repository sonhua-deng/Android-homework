...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(data!=null) {
            int position=data.getExtras().getInt("rPosition");
            if(position !=-1) {
                Items items=rItems.get(position);
                lItems.add(items);
            }
            else {
                position=data.getExtras().getInt("lPosition");
    ...
    public void initItems() {
        Items item1=new Items("Enchated Forest","¥ 5.00","作者","Johanna Basford",R.drawable.enchatedforest);
        rItems.add(item1);
        Items item2=new Items("Arla Milk","¥ 59.00","产地","德国",R.drawable.arla);
        rItems.add(item2);
        Items item3=new Items("Devondale Milk","¥ 79.00","产地","澳大利亚",R.drawable.devondale);
        rItems.add(item3);
        Items item4=new Items("Kindle Oasis","¥ 2399.00","版本","8GB",R.drawable.kindle);
        rItems.add(item4);
        Items item5=new Items("waitrose 早餐麦片","¥ 179.00","重量","2Kg",R.drawable.waitrose);
        rItems.add(item5);
        Items item6=new Items("Mcvitie's 饼干","¥ 14.90","产地","英国",R.drawable.mcvitie);
        rItems.add(item6);
        Items item7=new Items("Ferrero Rocher","¥ 132.59","重量","300g",R.drawable.ferrero);
        rItems.add(item7);
        Items item8=new Items("Maltesers","¥ 141.43","重量","118g",R.drawable.maltesers);
        rItems.add(item8);
        Items item9=new Items("Lindt","¥ 139.43","重量","249g",R.drawable.lindt);
        rItems.add(item9);
        Items item10=new Items("Borggreve","¥ 28.90","重量","640g",R.drawable.borggreve);
        rItems.add(item10);
    }
}